...
const CateringInfo = () => {
  ...
  const fetchCatering = async () => {
    try {
      ...
      showToast("Failed to load catering data from server", "danger");
      setCateringList([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleAddCatering = async (e) => {
    ...
    if (!cateringName.trim() || !ownerName.trim() || !contactNo.trim()) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setContactNo("");
        await fetchCatering();
        showToast("Catering profile registered successfully", "success");
      } else {
        const errorData = await response.json().catch(() => ({}));
        showToast(
          errorData.message || "Failed to save catering profile",
          "danger"
        );
      }
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      setSubmitting(true);
      const response = await fetch(`${API_URL}/catering/${id}`, {
        method: "DELETE",
      });

      if (response.ok) {
        await fetchCatering();
        showToast("Catering profile deleted successfully", "success");
      } else {
        showToast("Delete failed on server side", "danger");
      }
    } catch (error) {
  ...
  const handleSaveEdit = async (id) => {
    if (!editCatering.trim() || !editOwner.trim() || !editContact.trim()) {
    ...
    try {
      ...
      if (response.ok) {
        setEditingId(null);
        await fetchCatering();
        showToast("Catering profile updated successfully", "success");
      } else {
    ...
    <>
        ...
        <div className="row g-4">
          <div className="col-12">
            ...
            >
              ...
              <form onSubmit={handleAddCatering}>
              ...
              <div className="row g-3">
                        ...
                        <div className="card-body p-4 d-flex flex-column justify-content-between position-relative overflow-hidden">
                          ...
                          >
                            {isEditing ? (
                              <div className="mb-2">
                                ...
                                <input
                                  ...
                                  value={editCatering}
                                  onChange={(e) =>
                                    setEditCatering(e.target.value)
                                  }
                                />
                              </div>
                            ) : (
                              <h6
                                className="fw-bold text-dark mb-3 text-truncate-1"
                                style={{
                                  minHeight: "24px",
                                  lineHeight: "1.4",
                                  fontSize: "16px",
                            ...
                            {isEditing ? (
                              ...
                              <div className="mb-2 d-flex align-items-center text-secondary">
                            ...
                            {isEditing ? (
                              <div className="mb-3">
                                ...
                                <input
                                  type="text"
                                  className="form-control form-control-sm py-2 px-2 rounded-2 fw-semibold border-secondary"
                                  value={editContact}
                                  onChange={(e) =>
                ...
                {cateringList.length === 0 && (
                  <div className="col-12">
                    ...
                    >
                      No matching profiles detected. Add dynamic client routes!
                    </div>
                  </div>
                )}
              </div>
      ...
      >
        ...
        >
          ...
          >
            <FaTruck className="me-2" color="#F6643E" />
