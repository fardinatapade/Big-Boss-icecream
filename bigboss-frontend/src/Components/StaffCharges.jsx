...
const StaffCharges = () => {
  ...
  const fetchStaff = async () => {
    try {
      ...
      showToast("Failed to load staff from server", "danger");
      setStaffList([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleAddStaff = async (e) => {
    ...
    if (!staffName.trim() || !charge.toString().trim()) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCharge("");
        await fetchStaff();
        showToast("Staff profile added successfully", "success");
      } else {
        const errorData = await response.json().catch(() => ({}));
        showToast(
          errorData.message || "Failed to save staff profile",
          "danger"
        );
      }
    } catch (error) {
  ...
  const handleSaveEdit = async (id) => {
    if (!editName.trim() || !editCharge.toString().trim()) {
    ...
    <>
              ...
              <div className="row g-3">
                ...
                {staffList.length === 0 && (
                  <div className="col-12">
                    ...
                    >
                      No staff records found. Add new operational members!
                    </div>
                  </div>
                )}
              </div>
      ...
      >
        ...
        >
          ...
          >
            <FaUsers className="me-2" color="#F6643E" />
            <strong className="me-auto text-dark fw-bold">
              Big-Boss Staff Admin
            </strong>
            <button
              type="button"
              className="btn-close"
